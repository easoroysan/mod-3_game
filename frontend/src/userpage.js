class UserPage {
	...
	saveFile(file, count) {
		...
		deleteButton.addEventListener('click', () => {
			let deleteSavefile = confirm(
				'Are you sure you want to delete this savefile? If this is a high score, it will remove it from the high score board.'
			)
			if (deleteSavefile) {
				fetch(`${fetchURL}/savefiles/${file.id}`, { method: 'DELETE' }).then(
					() => div.remove()
				)
			}
		})
	...
	deleteAccount() {
		let deletion = confirm(
			'Are you sure you want to delete your account? This will delete all of your save files and high scores.'
		)
		if (deletion) {
